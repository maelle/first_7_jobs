...
first7jobs_parsed <- first7jobs  %>%
  ...
  filter(!grepl("\\#whatnext", text)) %>%
  filter(!grepl("[Tt]here are more", text)) %>%
  mutate(text = gsub("\\#[Ff]irstobs", "", text)) %>%
  mutate(text = gsub("\\#[Mm]y [fF]irst[sS]even[jJ]obs\\:", "", text)) %>%
  mutate(text = gsub("\\#[Mm]y [fF]irst[sS]even[jJ]obs", "", text)) %>%
  ...
  group_by(status_id) %>%
  # no empty
  filter(!grepl("[Mm]y", wordsgroup)) %>%
  filter(!grepl("\\:", wordsgroup)) %>%
  filter(!grepl("[Ll]ate", wordsgroup)) %>%
  filter(wordsgroup != "") %>%
  filter(wordsgroup != "  ") %>%
  filter(!grepl("   ", wordsgroup)) %>%
  filter(wordsgroup != " ") %>%
